	...
	return quizzes.flatMap((quiz) => {
		...
		for (let i = 1; i <= quiz.numberOfQuestions; i++) {
			...
			questions.push({
				...
				imageLink:
					"https://www.practicalnetworking.net/wp-content/uploads/2016/01/packtrav-osi-layers.png",
...
function generateQuestionText(title: string, index: number): string {
	switch (title) {
		...
		case "NAT & Port Forwarding":
			return index % 2 === 0 ? "Apa itu NAT?" : "Apa fungsi port forwarding?";

		case "VLAN & Trunking":
			return index % 2 === 0 ? "Apa itu VLAN?" : "Apa fungsi dari trunk port?";
...
export function generateMockQuestionsByQuiz(questions: Question[]): {
	[key: string]: Question[];
} {
